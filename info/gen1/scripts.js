...
let BattleScripts = {
  ...
  pokemon: {
    ...
    modifyStat(stat, modifier) {
      ...
      // @ts-ignore
      this.modifiedStats[stat] = this.battle.clampIntRange(
        Math.floor(this.modifiedStats[stat] * modifier),
        1,
        999
      );
    },
    ...
    boostBy(boost) {
      ...
      for (let i in boost) {
        ...
        stat = Math.floor(
          (Math.floor(
            2 * stat + this.set.ivs[i] + Math.floor(this.set.evs[i] / 4)
          ) *
            this.level) /
            100 +
            5
  ...
  runMove(moveOrMoveName, pokemon, targetLoc, sourceEffect) {
    ...
    if (
      ...
      !this.runEvent("BeforeMove", pokemon, target, move)
    ) {
    ...
    if (
      ...
      const moveSlot = pokemon.moveSlots.find(
        moveSlot => moveSlot.id === move.id
      );
  ...
  useMove(moveOrMoveName, pokemon, target, sourceEffect) {
    ...
    if (
      ...
      (move.category !== "Status" ||
        (move.status &&
          move.category === "Status" &&
          !["psn", "tox", "par"].includes(move.status))) &&
      ![
        "conversion",
        "haze",
        "mist",
  ...
  moveHit(target, pokemon, move, moveData, isSecondary, isSelf) {
    ...
    if (
      ...
      (doSelf ||
        (moveData.self !== true &&
          moveData.self.volatileStatus === "partialtrappinglock"))
    ) {
      ...
      for (const secondary of moveData.secondaries) {
        ...
        if (
          ...
          if (
            ...
            this.randomChance(Math.ceil((secondary.chance * 256) / 100), 256)
          ) {
  ...
  damage(damage, target, source, effect) {
    ...
    switch (effect.id) {
      case "partiallytrapped":
        this.add(
          ...
          target,
          target.getHealth,
          "[from] " + this.effectData.sourceEffect.fullname,
          "[partiallytrapped]"
        );
        break;
      default:
    ...
    if (effect.recoil && source) {
      this.damage(
        this.clampIntRange(
          Math.floor((damage * effect.recoil[0]) / effect.recoil[1]),
          1
        ),
        source,
        target,
    ...
    if (effect.drain && source) {
      this.heal(
        this.clampIntRange(
          Math.floor((damage * effect.drain[0]) / effect.drain[1]),
          1
        ),
        source,
        target,
  ...
  directDamage(damage, target, source, effect) {
    ...
    if (
      ["confusion", "highjumpkick"].includes(effect.id) &&
