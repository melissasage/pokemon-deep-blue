...
let BattleMovedex = {
  ...
  counter: {
    ...
    damageCallback: function(pokemon, target) {
      ...
      let lastUsedMove =
        target.side.lastMove && this.getMove(target.side.lastMove.id);
      if (
        lastUsedMove &&
  ...
  haze: {
    inherit: true,
    desc:
      "Resets the stat stages of both Pokemon to 0 and removes stat reductions due to burn and paralysis. Resets Toxic counters to 0 and removes the effect of confusion, Disable, Focus Energy, Leech Seed, Light Screen, Mist, and Reflect from both Pokemon. Removes the opponent's major status condition.",
  ...
  mirrormove: {
    inherit: true,
    desc:
      "The user uses the last move used by the target. Fails if the target has not made a move, or if the last move used was Mirror Move.",
  ...
  rest: {
    inherit: true,
    desc:
      "The user falls asleep for the next two turns and restores all of its HP, curing itself of any major status condition in the process. This does not remove the user's stat penalty for burn or paralysis. Fails if the user has full HP.",
    onHit: function(target) {
